...
"""Evaluate Blue Team Task B on a benchmark split with direct JSON writes.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate Blue Team Task B on a selected split.")
    parser.add_argument("--blue-run-path", type=Path, required=True)
    parser.add_argument("--blue-artifact-path", type=Path, required=True)
    parser.add_argument("--red-run-path", type=Path, required=True)
...
def main() -> None:
    ...
    sys.argv = [
        "evaluate_pipeline_validity.py",
        "--blue-run-path",
        str(args.blue_run_path.resolve()),
        "--blue-artifact-path",
        str(args.blue_artifact_path.resolve()),
        "--red-run-path",
        str(args.red_run_path.resolve()),
        "--red-eval-path",
        str(args.red_eval_path.resolve()),
        "--output-json",
        ...
        args.run_tag,
        "--source-run-tag",
        args.source_run_tag or args.run_tag,
        "--model-env-var",
        args.model_env_var,
        "--case-id",
        *case_ids,
