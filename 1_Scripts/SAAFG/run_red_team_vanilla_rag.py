...
DEFAULT_PROMPT_PATH = BASE_DIR / "3_Prompt" / "SAAFG" / "red_team_vanilla_rag.txt"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run SAAFG v0.2 VanillaRAG Red Team generation.")
...
def resolve_paths(args: argparse.Namespace, model_name: str) -> None:
    ...
    if args.result_dir is None:
        args.result_dir = DEFAULT_EXPERIMENT_ROOT / f"ma_VanillaRAG_{tag}" / "red_team"
    if args.output_path is None:
        args.output_path = args.result_dir / f"saafg_redteam_vanillarag_v0_2_{tag}.json"
    if args.artifact_path is None:
        args.artifact_path = args.result_dir / f"saafg_threat_records_pred_vanillarag_v0_2_{tag}.json"
    if args.retrieval_trace_path is None:
        args.retrieval_trace_path = args.result_dir / f"saafg_redteam_retrieval_trace_vanillarag_v0_2_{tag}.jsonl"
    if args.log_path is None:
        args.log_path = args.result_dir / f"saafg_redteam_vanillarag_v0_2_{tag}.log"
...
def retrieve_relevant_knowledge(case: Dict[str, Any], n_results: int) -> List[Dict[str, Any]]:
    if n_results <= 0:
        return []
...
def write_prediction_artifact(
    ...
    payload = {
        "meta": {
            "dataset_name": "SAAFG Red Team VanillaRAG predicted threat records",
            "version": "v0.2",
            ...
            "notes": [
                "Red Team receives retrieved external security knowledge from the local Chroma knowledge base.",
...
def main() -> None:
    ...
    meta = {
        **existing_meta,
        ...
        "task": "SAAFG Task A VanillaRAG Red Team generation",
        "version": "v0.2",
        "no_rag": False,
        "rag_enabled": True,
    ...
    for position, case in enumerate(selected_cases, start=1):
        ...
        retrieved_knowledge = retrieve_relevant_knowledge(case, args.rag_top_k)
