...
DEFAULT_PROMPT_PATH = BASE_DIR / "3_Prompt" / "SAAFG" / "red_team_no_rag.txt"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run SAAFG v0.2 NoRAG Red Team generation.")
...
def resolve_paths(args: argparse.Namespace, model_name: str) -> None:
    ...
    if args.result_dir is None:
        args.result_dir = DEFAULT_EXPERIMENT_ROOT / f"ma_NoRAG_{tag}" / "red_team"
    if args.output_path is None:
        args.output_path = args.result_dir / f"saafg_redteam_norag_v0_2_{tag}.json"
    if args.artifact_path is None:
        args.artifact_path = args.result_dir / f"saafg_threat_records_pred_norag_v0_2_{tag}.json"
    if args.log_path is None:
        args.log_path = args.result_dir / f"saafg_redteam_norag_v0_2_{tag}.log"
...
def validate_red_team_output(parsed: Any, expected_case_id: str, valid_bf_ids: Sequence[str]) -> Tuple[bool, List[str]]:
    ...
    expected_root_fields = {"use_case_id", "threat_records"}
    ...
    if parsed.get("use_case_id") != expected_case_id:
        errors.append(f"use_case_id mismatch: expected {expected_case_id}, got {parsed.get('use_case_id')}.")
...
def write_prediction_artifact(
    ...
    payload = {
        "meta": {
            "dataset_name": "SAAFG Red Team NoRAG predicted threat records",
            "version": "v0.2",
...
def main() -> None:
    ...
    meta = {
        **existing_meta,
        ...
        "task": "SAAFG Task A NoRAG Red Team generation",
        "version": "v0.2",
        "no_rag": True,
    ...
    for position, case in enumerate(selected_cases, start=1):
        ...
        result = run_red_team_case(llm, system_prompt, case, registry_row)
        result["index"] = position - 1
        results.append(result)
        completed_ids.add(case_id)
        status = "valid" if result.get("schema_valid") else "invalid"
        print(
            "[Done] {} status={} duration={:.3f}s threats={}".format(
                case_id,
                status,
