...
DEFAULT_PROMPT_PATH = BASE_DIR / "3_Prompt" / "SAAFG" / "blue_team_static_graph_rag.txt"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run SAAFG v0.2 StaticGraphRAG-GraphAware Blue Team generation."
    )
...
def resolve_paths(args: argparse.Namespace, model_name: str) -> Tuple[str, str]:
    ...
    if args.result_dir is None:
        args.result_dir = DEFAULT_EXPERIMENT_ROOT / f"ma_StaticGraphRAG_GraphAware_{blue_tag}" / "blue_team"
    if args.source_run_output_path is None:
        args.source_run_output_path = (
            DEFAULT_EXPERIMENT_ROOT
            / f"ma_StaticGraphRAG_GraphAware_{source_tag}"
            / "red_team"
            / f"saafg_redteam_staticgraphrag_graphaware_v0_2_{source_tag}.json"
        )
    if args.source_eval_output_path is None:
        args.source_eval_output_path = (
            ...
            / f"ma_StaticGraphRAG_GraphAware_{source_tag}"
            / "red_team"
            / f"saafg_redteam_task_a_eval_staticgraphrag_graphaware_v0_2_{source_tag}.json"
        )
    if args.output_path is None:
        args.output_path = args.result_dir / f"saafg_blueteam_staticgraphrag_graphaware_v0_2_{blue_tag}.json"
    if args.artifact_path is None:
        args.artifact_path = (
            args.result_dir / f"saafg_security_augmented_flows_pred_staticgraphrag_graphaware_v0_2_{blue_tag}.json"
        )
    if args.retrieval_trace_path is None:
        args.retrieval_trace_path = (
            args.result_dir / f"saafg_blueteam_retrieval_trace_staticgraphrag_graphaware_v0_2_{blue_tag}.jsonl"
        )
    if args.log_path is None:
        args.log_path = args.result_dir / f"saafg_blueteam_staticgraphrag_graphaware_v0_2_{blue_tag}.log"
...
def _format_graph_path(path: Dict[str, Any]) -> str:
    edges = path.get("edges") or []
    if not edges:
        return ""
    return " -> ".join(
        f"{edge.get('source_name')} -[{edge.get('relation')}]-> {edge.get('target_name')}"
        for edge in edges
    )


def _format_graphaware_blue_hint(item: Dict[str, Any]) -> str:
    graph_paths = item.get("graph_paths") or []
    if graph_paths:
        best_path = max(graph_paths, key=lambda path: float(path.get("path_score") or 0.0))
        best_path_text = _format_graph_path(best_path)
        return (
            "graph_reasoning_hint: Prefer this mitigation because it is connected to the matched "
            "Risk or AttackPattern through an explicit graph path. Adapt the Mitigation to the "
            "threat anchor BF step, and make the alternative flow trigger match the graph-supported "
            "attack or risk condition.\n"
            f"preferred_graph_path: {best_path_text}"
        )
    return (
        "graph_reasoning_hint: No explicit graph path was available for this mitigation; use it only "
        "as a semantic fallback if it directly mitigates the provided threat in the flow context."
    )


def _format_staticgraph_blue_document(item: Dict[str, Any]) -> str:
    mitigation = item.get("mitigation") or {}
    matched_sources = item.get("matched_sources") or []
    graph_paths = item.get("graph_paths") or []
    sections = [
        _format_graphaware_blue_hint(item),
        f"mitigation_name: {mitigation.get('name')}",
        f"mitigation_description: {mitigation.get('description')}",
        f"why_relevant: {item.get('why_relevant')}",
        f"matched_sources: {_join_names(matched_sources)}",
    ]
    if graph_paths:
        sections.append("graph_paths:")
        for path in graph_paths[:3]:
            path_text = _format_graph_path(path)
            if path_text:
...
def retrieve_relevant_knowledge(
    ...
    if n_results <= 0:
        return []
    result = retriever.retrieve_blue(flow_case, threat, top_k=n_results)
        ...
        retrieved_items.append(
            {
                ...
                "metadata": {
                    "dataset": mitigation.get("dataset"),
                    "id": mitigation.get("node_id"),
                    "node_type": mitigation.get("node_type"),
                    "source_title": (mitigation.get("metadata") or {}).get("source_title"),
                    "source_name": (mitigation.get("metadata") or {}).get("source_name"),
                },
                "document": _format_staticgraph_blue_document(item),
                "graph_item": item,
            }
        )
...
def write_artifact(
    ...
    payload = {
        "meta": {
            "dataset_name": "SAAFG Blue Team StaticGraphRAG-GraphAware predicted security augmented flows",
            "version": "v0.2",
            ...
            "notes": [
                "Blue Team receives ranked mitigation evidence from the static Rec-EvoGraph-RAG knowledge graph.",
...
def main() -> None:
    ...
    meta = {
        **existing_meta,
        ...
        "task": "SAAFG Task B StaticGraphRAG-GraphAware Blue Team generation",
        "version": "v0.2",
        "no_rag": False,
        "rag_enabled": True,
        ...
        "generation_policy": (
            ...
            "Mitigation path prompt guidance, no retries, no JSON repair, no schema correction, "
    ...
    for position, source_case_result in enumerate(selected_source_results, start=1):
        ...
        for threat_index, threat in enumerate(threat_records, start=1):
            ...
            retrieved_knowledge = retrieve_relevant_knowledge(retriever, flow_case, threat, args.rag_top_k)
