...
"""Run SAAFG v0.2 Rec-EvoGraphRAG Blue Team generation.

This wrapper reuses the verified StaticGraphRAG-GraphAware Blue runner while
switching to the evolved retriever/KG and Rec-EvoGraph output conventions.
Existing baseline scripts are not modified.
"""
...
def resolve_paths(args: Any, model_name: str) -> tuple[str, str]:
    ...
    if args.result_dir is None:
        args.result_dir = DEFAULT_EXPERIMENT_ROOT / f"ma_RecEvoGraphRAG_{blue_tag}" / "blue_team"
    if args.source_run_output_path is None:
        args.source_run_output_path = (
            DEFAULT_EXPERIMENT_ROOT
            / f"ma_RecEvoGraphRAG_{source_tag}"
            / "red_team"
            / f"saafg_redteam_recevographrag_v0_2_{source_tag}.json"
        )
    if args.source_eval_output_path is None:
        args.source_eval_output_path = (
            ...
            / f"ma_RecEvoGraphRAG_{source_tag}"
            / "red_team"
            / f"saafg_redteam_task_a_eval_recevographrag_v0_2_{source_tag}.json"
        )
    if args.output_path is None:
        args.output_path = args.result_dir / f"saafg_blueteam_recevographrag_v0_2_{blue_tag}.json"
    if args.artifact_path is None:
        args.artifact_path = (
            args.result_dir / f"saafg_security_augmented_flows_pred_recevographrag_v0_2_{blue_tag}.json"
        )
    if args.retrieval_trace_path is None:
        args.retrieval_trace_path = (
            args.result_dir / f"saafg_blueteam_retrieval_trace_recevographrag_v0_2_{blue_tag}.jsonl"
        )
    if args.log_path is None:
        args.log_path = args.result_dir / f"saafg_blueteam_recevographrag_v0_2_{blue_tag}.log"
...
def patch_meta(meta: Dict[str, Any]) -> Dict[str, Any]:
    ...
    patched.update(
        {
            "task": "SAAFG Task B RecEvoGraphRAG Blue Team generation",
            ...
            "generation_policy": (
                ...
                "graph-aware Risk/AttackPattern -> Mitigation path prompt guidance, no retries, "
...
def install_patches() -> None:
    ...
    def write_artifact(
        ...
        payload["meta"]["dataset_name"] = "SAAFG Blue Team RecEvoGraphRAG predicted security augmented flows"
        payload["meta"]["rag_method"] = "RecEvoGraphRAG"
        notes = payload["meta"].get("notes") or []
        if notes:
            notes[0] = "Blue Team receives ranked mitigation evidence from the critic-evolved Rec-EvoGraph-RAG knowledge graph."
