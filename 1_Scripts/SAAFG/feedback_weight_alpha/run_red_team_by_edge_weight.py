...
"""Run RecEvoGraphRAG Red Team generation for feedback_weight_alpha sensitivity."""
...
import run_red_team_ma_safr as base_runner
...
def resolve_paths(args: Any, model_name: str) -> None:
    tag = resolve_model_tag(args, model_name)
    if args.result_dir is None:
        args.result_dir = experiment_dir(CURRENT_ALPHA, tag, "red_team")
    if args.output_path is None:
        args.output_path = args.result_dir / f"saafg_redteam_recevographrag_{CURRENT_ALPHA_TAG}_v0_2_{tag}.json"
    if args.artifact_path is None:
        args.artifact_path = args.result_dir / f"saafg_threat_records_pred_recevographrag_{CURRENT_ALPHA_TAG}_v0_2_{tag}.json"
    if args.retrieval_trace_path is None:
        args.retrieval_trace_path = (
            args.result_dir / f"saafg_redteam_retrieval_trace_recevographrag_{CURRENT_ALPHA_TAG}_v0_2_{tag}.jsonl"
        )
    if args.log_path is None:
        args.log_path = args.result_dir / f"saafg_redteam_recevographrag_{CURRENT_ALPHA_TAG}_v0_2_{tag}.log"
